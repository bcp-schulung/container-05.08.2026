...
module "kubernetes" {
  ...
  cluster_name = "container-seminar-0508"
  ...
  # Allow the Kubernetes API and Talos API from anywhere for seminar access.
  firewall_kube_api_source  = ["0.0.0.0/0", "::/0"]
  firewall_talos_api_source = ["0.0.0.0/0", "::/0"]

  # Expose everything to everyone: all TCP/UDP ports and ICMP open from any source.
