...
# Tried in order if the preferred location reports resource_unavailable
# (Hetzner occasionally runs out of cx23 capacity in a single location).
FALLBACK_LOCATIONS = [LOCATION_NAME, "fsn1", "nbg1"]
...
STUDENTS = [
    {"slug": "annika-schellschmidt", "display": "Annika Schellschmidt"},
    {"slug": "ben-coeppicus",        "display": "Ben Cöppicus"},
    {"slug": "carolin-schlegel",     "display": "Carolin Schlegel"},
    {"slug": "ghazal-ghazal",        "display": "Ghazal Ghazal"},
    {"slug": "helge-carsten-buer",   "display": "Helge Carsten Buer"},
    {"slug": "juergen-bendfeldt",    "display": "Jürgen Bendfeldt"},
    {"slug": "lars-kowoll",          "display": "Lars Kowoll"},
    {"slug": "marc-kaesler",         "display": "Marc Kaesler"},
    {"slug": "pascal-brand",         "display": "Pascal Brand"},
    {"slug": "sergej-holzer",        "display": "Sergej Holzer"},
    {"slug": "stefan-jungen",        "display": "Stefan Jungen"},
    {"slug": "thomas-koos",          "display": "Thomas Koos"},
    {"slug": "thomas-siewert",       "display": "Thomas Siewert"},
]
...
hc = HCloudClient(token=HCLOUD_TOKEN, timeout=30)
...
def create_vm(name: str, ssh_key: object) -> tuple[str, str]:
    ...
    # the whole rotation is repeated a few times in case capacity frees up.
    attempts_per_location = 2
    max_rounds = 3
    response = None
    attempt = 0
    for round_num in range(1, max_rounds + 1):
        for location_name in FALLBACK_LOCATIONS:
            for _ in range(attempts_per_location):
                attempt += 1
                try:
                    ...
                    if e.code == "resource_unavailable":
                        log(
                            f"  [{name}] '{location_name}' unavailable "
                            f"(attempt {attempt}), trying next location ..."
                        )
                        time.sleep(10)
                        continue
                    raise
    ...
    if response is None:
        raise RuntimeError(
            f"Server '{name}' could not be created after trying "
            f"{FALLBACK_LOCATIONS} {max_rounds} times each"
        )
...
def provision_hetzner() -> dict[str, str]:
    ...
    vm_ips: dict[str, str] = {}
    # Keep concurrency low: creating many servers at once against the same
    # location/server-type tends to trigger Hetzner's resource_unavailable
    # response, and floods the shared HTTP connection pool.
    with ThreadPoolExecutor(max_workers=min(4, len(STUDENTS))) as pool:
        futures = {
